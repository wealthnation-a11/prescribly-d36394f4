...
export const SystemAssessment = () => {
  ...
  const shareViaWhatsApp = () => {
    ...
    if (diagnosisResults && diagnosisResults.length > 0) {
      ...
      diagnosisResults.forEach(diagnosis => {
        message += `- Condition: ${diagnosis.condition}\n`;
        message += `  Confidence: ${diagnosis.confidence}%\n`;
        
        const recommendations = drugRecommendations[diagnosis.condition];
        if (recommendations && recommendations.length > 0) {
          message += `  Recommended Drug: ${recommendations[0].medication}\n`;
        }
      });
    }
  ...
  const shareViaEmail = () => {
    ...
    if (diagnosisResults?.diagnosis && Array.isArray(diagnosisResults.diagnosis)) {
      body += `\nDiagnosis Results:\n`;
      diagnosisResults.diagnosis.forEach((condition: any, index: number) => {
        const probability = condition.confidence ? ` (${Math.round(condition.confidence * 100)}% confidence)` : '';
        body += `- Condition: ${condition.name}${probability}\n`;
    ...
    body += `\nImportant: Always consult a healthcare professional before taking any medication.\n\n`;
    body += `Generated by PrescriblyAI`;
