...
ALTER TABLE IF EXISTS public.prescriptions_v2 DROP CONSTRAINT IF EXISTS fk_prescriptions_diagnosis;
ALTER TABLE IF EXISTS public.audit_logs DROP CONSTRAINT IF EXISTS fk_audit_diagnosis;

-- Create diagnosis_sessions_v2 table with requested schema
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'fk_prescriptions_v2_diagnosis' 
        AND table_name = 'prescriptions_v2'
    ) THEN
        ALTER TABLE public.prescriptions_v2 
        ADD CONSTRAINT fk_prescriptions_v2_diagnosis 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_audit_logs_diagnosis' 
        AND table_name = 'audit_logs'
    ) THEN
        ALTER TABLE public.audit_logs 
        ADD CONSTRAINT fk_audit_logs_diagnosis 
        FOREIGN KEY (diagnosis_id) REFERENCES public.diagnosis_sessions_v2(id) ON DELETE CASCADE;
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS update_diagnosis_sessions_v2_updated_at ON public.diagnosis_sessions_v2;
...
DROP POLICY IF EXISTS "Users can create their own diagnosis sessions" ON public.diagnosis_sessions_v2;
DROP POLICY IF EXISTS "Users can view their own diagnosis sessions" ON public.diagnosis_sessions_v2;
DROP POLICY IF EXISTS "Users can update their own diagnosis sessions" ON public.diagnosis_sessions_v2;
DROP POLICY IF EXISTS "Doctors can view diagnosis sessions for review" ON public.diagnosis_sessions_v2;
DROP POLICY IF EXISTS "Doctors can update diagnosis sessions for review" ON public.diagnosis_sessions_v2;
...
-- Drop existing policies for prescriptions_v2
...
DROP POLICY IF EXISTS "Doctors can update their own prescriptions" ON public.prescriptions_v2;
...
DROP POLICY IF EXISTS "Users can view audit logs for their diagnosis sessions" ON public.audit_logs;
DROP POLICY IF EXISTS "Doctors can view audit logs for sessions they're involved in" ON public.audit_logs;
...
-- Drop existing policies for condition_drug_map
DROP POLICY IF EXISTS "Authenticated users can view condition drug mappings" ON public.condition_drug_map;
DROP POLICY IF EXISTS "Admins can manage condition drug mappings" ON public.condition_drug_map;
DROP POLICY IF EXISTS "Doctors can view condition drug mappings" ON public.condition_drug_map;
